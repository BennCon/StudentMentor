...
RSpec.describe User do
    describe "#load" do
        it "loads an user object with information about said mentor" do
            user = described_class.new(email: "123@AB.CDE", password: "pass", first_name: "A", surname: "B", user_type: "mentee", username:"user")
            user.save_changes
            expect(user.values).to eq({:email=>"123@AB.CDE", :password=>"pass", :first_name=> "A", :surname=> "B", :user_type=> "mentee", :id=>1, :username=>"user"})
            user.delete
        end
    end
